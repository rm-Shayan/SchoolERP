...
export default function BranchTimetablePage() {
  ...
  const { data: slots = [], isLoading } = useGetBySectionQuery(sectionId, { skip: !sectionId });
  const [deleteSlot] = useDeleteSlotMutation();
  const [clearSlots] = useClearAllSlotsMutation();
  ...
  const handleClear = async () => {
    if (!sectionId) return;
    try { const r = await clearSlots({ sectionId, dayOfWeek: clearDay ? Number(clearDay) : undefined }).unwrap(); toast.success(`${r.deletedCount} cleared`); setShowClear(false); setClearDay(''); }
  ...
  return (
    <div className="space-y-6">
      ...
      {sectionId && !isLoading && slots.length > 0 && <TimetableStats slots={slots} todayDow={todayDow} />}
      {sectionId && !isLoading && slots.length > 0 && (
      ...
      <TimetableSlotModal key={editing?.id ?? 'new'} open={showModal} sectionId={sectionId} schoolId={schoolId} editing={editing}
        onClose={() => { setShowModal(false); setEditing(null); }} onSaved={() => { setShowModal(false); setEditing(null); }} />
      <TimetableImportModal open={showImport} sectionId={sectionId} onClose={() => setShowImport(false)} onImported={() => setShowImport(false)} />
      ...
      : selectedDay ? (
        ...
        : <DraggableDaySlots slots={sortedSlots} sectionId={sectionId} dayOfWeek={selectedDay}
            onReordered={() => {}} onEdit={(s) => { setEditing(s); setShowModal(true); }} onDelete={setDeleting} />
      ) : (
      ...
      <ConfirmDialog open={Boolean(deleting)} title="Delete slot?" message="This slot will be permanently removed."
        confirmLabel="Delete" loading={false} onConfirm={handleDelete} onCancel={() => setDeleting(null)} />
      <ConfirmDialog open={showClear} title="Clear timetable?" message="Select which slots to clear."
        confirmLabel={clearDay ? `Clear ${DAY_NAMES[Number(clearDay)]}` : 'Clear All'} loading={false} onConfirm={handleClear}
