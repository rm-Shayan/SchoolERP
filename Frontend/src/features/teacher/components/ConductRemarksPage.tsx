...
import RemarkForm from './parts/RemarkForm';
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={editing ? 'Edit Remark' : 'Conduct Remarks'}
        description={editing ? 'Update the remark below.' : 'Quick 10-second behaviour note — instantly emailed to parents.'} />

      <RemarkForm editing={editing} onSaved={() => { setEditing(null); loadRemarks(); }} onCancel={() => setEditing(null)} />

      <div>
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Your Remarks History ({remarkTotal})</h3>
        {remarks.length === 0 ? (
          <Card><EmptyState title="No remarks yet" description="Your remarks will appear here." /></Card>
        ) : (
