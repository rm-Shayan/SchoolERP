...
export default function TeacherPasswordTab() {
  const { values, errors, isSubmitting, handleChange, handleBlur, handleSubmit, reset } = useForm({
    ...
    onSubmit: async (v) => {
      try {
        await authService.changePassword({ oldPassword: v.oldPassword as string, newPassword: v.newPassword as string });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input label="Current Password" type="password" name="oldPassword" placeholder="••••••••" value={values.oldPassword as string} onChange={handleChange} onBlur={() => handleBlur('oldPassword')} error={errors.oldPassword} required />
        <div>
          <Input label="New Password" type="password" name="newPassword" placeholder="••••••••" value={values.newPassword as string} onChange={handleChange} onBlur={() => handleBlur('newPassword')} error={errors.newPassword} required />
          {values.newPassword ? (
            <div className="mt-2">
        ...
        <Input label="Confirm New Password" type="password" name="confirmPassword" placeholder="••••••••" value={values.confirmPassword as string} onChange={handleChange} onBlur={() => handleBlur('confirmPassword')} error={errors.confirmPassword} required />
