...
export default function AttendanceTable({ students, marks, percentages, onToggle, onBulkSet }: Props) {
  ...
  const badge = (s: AttendanceStatus, id: string, active: boolean) => {
    ...
    const cls: Record<string, string> = { PRESENT: 'bg-emerald-600 text-white ring-emerald-300', LATE: 'bg-yellow-500 text-white ring-amber-300', ABSENT: 'bg-red-600 text-white ring-red-300', LEAVE: 'bg-blue-500 text-white ring-blue-300' };
    return (
  ...
  const avatarColor = (st?: string) => st === 'PRESENT' ? 'bg-emerald-100 text-emerald-700' : st === 'LATE' ? 'bg-amber-100 text-amber-700' : st === 'ABSENT' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700';
