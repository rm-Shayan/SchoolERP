...
export function AdmissionsTable({ applicants, total, refetching, onView, onEdit, onAdd, pageSize, onPageSizeChange }: AdmissionsTableProps) {
    ...
    <Card className="overflow-hidden border-gray-200/80 shadow-sm">
      <CardHeader className="flex flex-col gap-3 bg-gradient-to-r from-primary-50/60 via-white to-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div className="flex items-center gap-2">
          <div><h2 className="font-bold text-gray-900">Applicants</h2><p className="mt-0.5 text-xs text-gray-500">Review and move applicants through the pipeline</p></div>
          <Badge variant="info">{total}</Badge>
        </div>
        <div className="flex items-center gap-4">
        {pageSize <= 5 && <button type="button" onClick={() => onPageSizeChange(10)} className="group inline-flex items-center gap-1 text-xs font-bold text-primary-700 hover:text-primary-800">Expand list<svg className="h-3.5 w-3.5 transition-transform group-hover:translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg></button>}
        {refetching && (
        ...
        <table className="w-full min-w-[760px] text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {applicants.map((applicant) => (
              <AdmissionsRow key={applicant.id} applicant={applicant} onView={onView} onEdit={onEdit} />
            ))}
          </tbody>
        </table>
      </div>
