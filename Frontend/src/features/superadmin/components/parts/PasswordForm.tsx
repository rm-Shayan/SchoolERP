...
export default function PasswordForm() {
  const { values, errors, isSubmitting, handleChange, handleBlur, handleSubmit, reset } = useForm({
    ...
    validators: {
      ...
      confirmPassword: composeValidators(
        ...
        (value, all) => (value === all.newPassword ? undefined : 'New passwords do not match')
      ),
    },
    onSubmit: async (v) => {
      try {
  ...
  const strength: Strength = useMemo(() => {
    const value = values.newPassword as string;
    if (!value) return { width: '0%', bar: 'bg-gray-200', label: '', text: 'text-gray-400' };
    let score = 0;
