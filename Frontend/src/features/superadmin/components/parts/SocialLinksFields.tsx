      ...
      <div className="space-y-3">
        {SOCIAL_FIELDS.map((s) => (
          <div key={s.name} className="flex items-center gap-3">
            <svg className={`w-5 h-5 shrink-0 ${s.color}`} viewBox="0 0 24 24" fill="currentColor"><path d={s.d} /></svg>
            <Input name={s.name} placeholder={s.placeholder} value={values[s.name] ?? ''} onChange={onChange} />
          </div>
        ))}
      </div>
    </div>
