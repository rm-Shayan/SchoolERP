...
export default function AuthLayout({
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        'relative mx-auto flex w-full flex-col justify-center',
        'max-w-6xl',
        compact ? 'min-h-[calc(100vh-2rem)] px-3' : 'min-h-[calc(100vh-4rem)] px-3 sm:px-0',
      )}>
        <motion.div {...cardMotion} initial={false} className={cn(
          ...
          fullWidth ? 'gap-0 rounded-none' : 'lg:grid-cols-[1.05fr_0.95fr]',
        )}>
          {!fullWidth && (
            <BrandPanel
          ...
          <motion.div initial={false}
            className={cn(
              'flex flex-col justify-start p-4 sm:p-6 sm:p-8 lg:p-10 lg:pt-12',
...
function MobileHeader({ branded, compact, brandIcon, brandSub, brandLabel, themeColor, resolvedSlug }: {
  ...
  return (
    <motion.div initial={false} className={cn('text-center lg:hidden', compact ? 'mb-3' : 'mb-4 sm:mb-6')}>
      {branded ? (
        <>
          <div className="mx-auto flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center overflow-hidden rounded-2xl bg-white p-1.5 shadow-lg ring-1 ring-gray-200">{brandIcon}</div>
