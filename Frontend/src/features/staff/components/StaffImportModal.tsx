...
export default function StaffImportModal({ open, onClose, onImported }: StaffImportModalProps) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const { jobId, totalRows } = await staffService.importExcel(file);
      toast.success(`Import started — ${totalRows} rows queued (job ${jobId})`);
      onImported();
    } catch (err: any) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Bulk Staff Import" size="lg">
      <div className="space-y-4">
        ...
        >
          ...
          <p className="text-xs text-gray-500 mt-1">Each row creates a staff account and emails credentials.</p>
        </div>
