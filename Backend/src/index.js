...
const shutdown = async (signal) => {
  ...
  try {
    ...
    setDraining();
    console.log(
      `   ⏳ Draining for ${DRAIN_TIMEOUT_MS / 1000}s — /health returning 503`,
    );
...
const start = async () => {
  ...
  console.log("╚══════════════════════════════════════════════╝");
  console.log(
    `  ⏱  Module imports:      ${ms(T_MODULE_IMPORTS_DONE)} (from process start)`,
  );
  ...
  const dbReady = prisma.$queryRaw`SELECT 1`
  ...
  const { warmupOrgStorageCache } =
    await import("./lib/utils/orgStorage.cache.js");
